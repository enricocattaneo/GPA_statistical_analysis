---
title: 'College GPA: between Gender and IQ/Preparation.'
...
```{r}
sat_gpa$gpa_hs = factor(sat_gpa$gpa_hs, levels = c("low", "high"), labels = c("L", "H"))
sat_gpa$sex = factor(sat_gpa$sex, levels = c("Male", "Female"), labels = c("M", "F"))
sat_gpa = sat_gpa %>% rename(id = '...1')
```

### Problem Description
##### Does gender influence College GPA? Is the SAT score a valuable indication of future GPA?
In our analysis, we will investigate the differences in GPA between genders (binary), and the relationship between college GPA and SAT scores (taken during High School). Considering the SAT as an approximation of the IQ (or in a more precise way, of past preparation) we can expect a positive relationship between this variable and the GPA. While the relationship between GPA and gender is of less intuitive interpretation. We will study our sample data (considered as randomly selected and representative) for a better understanding of these relationships' signs and values between those variables. 

### Response and Explanatory Variables 
Response: gpa_py (numerical variable).
Explanatory: sat_verbal (numerical variable),
...
##### 1) Raw Data
```{r}
data_model1 = sat_gpa %>% # save important variables into a new data frame
  select(id, gpa_fy, sex, sat_verbal)
glimpse(data_model1)
data_model1 %>% sample_n(size = 5)
```
##### 2) Computing Summary Statistics
```{r}
data_model1 %>% skim() 
data_model1 %>% get_correlation(formula = gpa_fy ~ sat_verbal) #corr between numerical variables 
```
...
data_model1 %>% 
  ggplot(aes(x = gpa_fy)) +
  geom_histogram(color = "white", fill = "orange", bins = 15) +
  labs(x = "GPA_fy", title = "GPA first collage year")
skewness(data_model1$gpa_fy) # approximately symmetric (because between -0.50 and 0.50)

# histogram sat_verbal
data_model1 %>% 
  ggplot(aes(x = sat_verbal)) +
  geom_histogram(color = "white", fill = "red", bins = 25) +
  labs(x = "sat_verbal", title = "SAT verbal score (percentile)")
skewness(data_model1$sat_verbal) # approximately symmetric (because between -0.50 and 0.50)

# barplot sex
data_model1 %>% 
  ggplot() +
  geom_bar(aes(x = sex, fill = sex)) +
  labs(x = "Sex", title = "Gender Distribution") +
...
data_model1 %>% 
  ggplot(aes(x = sat_verbal, y = gpa_fy, col = sex)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "lm", se = F) +
  labs(y = "GPA first year",
       x = "SAT verbal",
       title = "First year College Results") +
  scale_color_manual(values=c("#3399FF", "#FF6666"))
...
data_model1 %>% 
  ...
  labs(y = "GPA first year",
       x = "SAT verbal",
       title = "First year of College Results") +
  scale_color_manual(values=c("#3399FF", "#FF6666"))
```

### MODEL
We start from two simple linear regression and conclude our analysis with a multiple regression model.

#### SIMPLE LINEAR REGRESSION  WITH GPA_FY ~ SAT_VERBAL
```{r}
simple_model_sat = lm(gpa_fy ~ sat_verbal, data = data_model1) # fit regression model
get_regression_table(simple_model_sat) # get regression table
```

##### Confidence Intervals 

```{r}
bootstrap_slope = data_model1 %>%  # bootstrap slope
  specify(gpa_fy ~ sat_verbal) %>% 
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "slope")

percentile_ci = bootstrap_slope %>% # percentile method 
  get_confidence_interval(type = "percentile", level = 0.95)
percentile_ci

observed_slope = data_model1 %>% # standard error method (normality assumption)
  specify(gpa_fy ~ sat_verbal) %>% 
  calculate(stat = "slope")
se_ci = bootstrap_slope %>% 
...
visualize(bootstrap_slope)+ 
  ...
  shade_confidence_interval(endpoints = c(0.031, 0.041), fill = NULL, 
...
```{r}
# null distribution
null_distribution_gpa_sat = data_model1 %>% 
  specify(gpa_fy ~ sat_verbal) %>% 
  hypothesize(null = "independence") %>% 
  generate(reps = 10000, type = "permute") %>% 
  calculate(stat = "slope")

obs_slope = data_model1 %>% # observed slope
  specify(formula = gpa_fy ~ sat_verbal) %>% 
  calculate(stat = "slope")

visualize(null_distribution_gpa_sat) + # visualize the p-value
  shade_p_value(obs_stat = obs_slope, direction = "both")

null_distribution_gpa_sat %>% # what percentage of the null distr. is shaded?
  get_p_value(obs_stat = obs_slope, direction = "both")
# anova
anova(simple_model_sat)
```
